...
#heart_eval <-(predict(heart_3NN,newdata = test))#generates 1s and 0s
heart_eval_prob = heart_3NN
heart_eval = heart_3NN


#heart_eval_prob <- predict(loan_tree,newdata = test, type = "prob")#this gives us the predicted prob, we will need these later for the fairness evaluation
#View(heart_eval_prob)

#View(test$`DEATH_EVENT`)
heart_eval = as.factor(heart_eval[2:length(heart_eval)])
table(heart_eval, test$`DEATH_EVENT`)#essential the confusion matrix, though we can make a fancy one using caret built in functions

#length(heart_eval[2:length(heart_eval)])
#length(test$`DEATH_EVENT`)
str(heart_eval)
#print(heart_eval)
str(test$`DEATH_EVENT`)

confusionMatrix(heart_eval[2:length(heart_eval)], test$`DEATH_EVENT`, positive = "1", dnn=c("Prediction", "Actual"), mode = "sens_spec")
?confusionMatrix
